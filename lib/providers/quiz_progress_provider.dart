...
final quizProgressNotifierProvider = StateNotifierProvider.family<QuizProgressNotifier, bool, QuizProgressServiceArgs>((ref, args) {
  final quizProgressService = ref.watch(quizProgressServiceProvider(args));
  return QuizProgressNotifier(quizProgressService: quizProgressService, ref: ref, uid: args.uid, level: args.level, correctAnswers: args.correctAnswers, totalQuestions: args.totalQuestions);
});

final quizProgressProvider = FutureProvider.family<QuizProgressModel?, QuizProgressServiceArgs>((ref, args) async {
  final quizProgressService = ref.watch(quizProgressServiceProvider(args));
...
class QuizProgressNotifier extends StateNotifier<bool>{
  ...
  QuizProgressNotifier({required quizProgressService, required ref, required this.uid, required this.level, required this.correctAnswers, required this.totalQuestions})
